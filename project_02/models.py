...
class SmoothClassifier(nn.Module):
    ...
    def certify(self, inputs: torch.Tensor, n0: int, num_samples: int, alpha: float, batch_size: int) -> Tuple[int,
        ...
        ##########################################################
        # YOUR CODE HERE
        most_likely_class = self._sample_noise_predictions(inputs,n0,batch_size)
        top_class = most_likely_class.argmax().cpu()
        p_A_lower_bound = lower_confidence_bound(self._sample_noise_predictions(inputs,num_samples,batch_size)[top_class].cpu(), num_samples, alpha)
    ...
    def predict(self, inputs: torch.tensor, num_samples: int, alpha: float, batch_size: int) -> int:
        ...
        ##########################################################
        # YOUR CODE HERE
        first_two = class_counts.argsort(descending=True)[:2]
        count1 = class_counts[first_two[0]]
        count2 = class_counts[first_two[1]]
    ...
    def _sample_noise_predictions(self, inputs: torch.tensor, num_samples: int, batch_size: int) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            for it in range(ceil(num_samples / batch_size)):
                ...
                logits = self.base_classifier(torch.clip(batch + torch.randn_like(batch), 0, 1).to(self.base_classifier.device()))
