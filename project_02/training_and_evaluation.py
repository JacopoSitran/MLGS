...
def train_model(model: nn.Module, dataset: Dataset, batch_size: int, loss_function: Callable, optimizer: Optimizer,
    ...
    for epoch in range(epochs):
        ...
        for x,y in tqdm(train_loader, total=num_train_batches):
            ...
            # YOUR CODE HERE
            if len(loss_args) > 0:
                loss, logits = loss_function(x.to(model.device()),y,model,attack_args=loss_args)
            else:
                loss, logits = loss_function(x.to(model.device()),y,model)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            losses.append(loss)
            logits = torch.argmax(logits, dim = 1)
            accuracies.append(torch.sum(logits == y)/y.shape[0])
            ##########################################################
...
def evaluate_robustness_smoothing(base_classifier: nn.Module, sigma: float, dataset: Dataset,
    ...
    for x, y in tqdm(test_loader, total=len(dataset)):
        ##########################################################
        # YOUR CODE HERE
        prediction, radius = model.certify(x,num_samples_1, num_samples_1, alpha,certification_batch_size)
        correct_certified += int(prediction == y)
        false_predictions += int(prediction != y)
        abstains += int(prediction == torch.ones(len(y))*-1)
        radii.append(radius)
        ##########################################################
