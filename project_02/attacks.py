...
def fast_gradient_attack(logits: torch.Tensor, x: torch.Tensor, y: torch.Tensor, epsilon: float, norm: str = "2",
    ...
    loss.backward()
    gradient_norm = torch.sign(x.grad) if norm == 'inf' else x.grad/torch.linalg.norm(x.grad, int(norm), dim=[2, 3], keepdims=True)
    x_pert = torch.clip(x + epsilon * gradient_norm, 0, 1)
